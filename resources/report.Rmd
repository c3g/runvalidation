...
if(run.is.demultiplexed) {
  ...
  runinfo <- tibble(
    ...
    "Total read number"     = fqstats$ReadNum %>% sum %>% divide_by(1) %>% round(1) %>% scales::comma(),
    "Total bases (Mbp)"     = fqstats$BaseNum %>% sum %>% scales::comma(scale = 1/1e6),
    "Total PF clusters(G)"  = "N/A (run not demultiplexed)",
...
rsmetrics %>%
  ...
  mutate(
    ...
    `index.% on index in lane` = ifelse(run.is.demultiplexed, `index.% on index in lane` %>% sprintf(fmt = "%.1f"), "100"),
    `index.% of the lane` = ifelse(run.is.demultiplexed, `index.% of the lane` %>% sprintf(fmt = "%.1f"), "100%"),
