...
list.files("demux_metrics", full.names = TRUE) %>%
  lapply(function(path) {
    lane <- str_match(path, "([0-9]).DemuxFastqs.metrics.txt")[, 2]
    read_tsv(path) %>% mutate(lane = lane)
  }) %>%
  do.call(what = rbind) %>%
  mutate(
  ...
  arrange(lane, templates %>% desc()) -> barcode_metrics
```
...
barcode_metrics %>%
  ...
  reactable(
    ...
    columns = list(
      lane = colDef(name = "Lane", align = "left", maxWidth = 50),
      library_name = colDef(name = "Library", align = "right"),
      sample = colDef(name = "Sample", align = "right"),
      barcode_simple = colDef(name = "Barcode Name", align = "right", minWidth = 150),
      barcode = colDef(name = "Barcode", align = "right"),
      templates = colDef(name = "Templates", align = "right"),
      ...
      pf_normalized_matches = colDef(name = "Normalized Matches", align = "right")
    ),
    rowStyle = JS(
      "
    function(rowInfo, state) {
      ...
      if (rowInfo['row']['sample'] == 'unmatched') {
        return { backgroundColor: '#ea3a3a14' }
      } else {
