...
barcode_metrics <- list.files("demux_metrics", full.names = TRUE) %>%
...
```{r, fig.dim=c(8, 3),echo=F,messages=F,warnings=F}
barcode_metrics %>%
  mutate(fraction_matches = sprintf("%.3f", fraction_matches),
  ...
  reactable(
    ...
    columns = list(
      ...
      barcode_name = colDef(name = "Barcode Name", align = "right", minWidth = 250),
