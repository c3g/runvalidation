...
parse.eventfile.v1 <- function(filename) {
  filename %>%
    ...
    mutate(
      ...
      Species = replace_na(Species, "Not provided")
    ) %>%
    ...
    lapply(function(filename) {
      ...
      meta <- read_tsv(filename, n_max = 12, col_names = c("key", "value")) %>%
        ...
        mutate(
          ...
          Read = str_match(Name, ".*_L[0-9]+_.*_([12]).fq.gz")[, 2],
          Sample = str_match(Name, ".*_L[0-9]+_(.*)_[12].fq.gz")[, 2]
        )
    }) %>%
    do.call(what = rbind) %>%
    relocate(Lane, Read) %>%
...
column_definitions = list(
  ...
  "Sample name" = colDef(
    minWidth = 100 + tmp.samplename.max * 5,
  ...
  "Expected species"  = colDef(minWidth = 100),
